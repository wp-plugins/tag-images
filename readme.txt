...
In order to make use of the image, you must alter your theme to include it where appropriate. In it's simplest case, this is simply something like `echo get_tag_image($tag)` or `foreach (get_the_tags() as $tag) echo get_tag_image($tag);`, or something.
